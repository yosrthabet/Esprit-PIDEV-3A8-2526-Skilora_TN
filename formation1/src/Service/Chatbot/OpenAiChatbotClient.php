...
/**
 * Calls OpenAI Chat Completions with a system prompt built from live {@see Formation} catalogue data.
 */
final class OpenAiChatbotClient implements ExternalAiClientInterface
{
    ...
    /**
     * @param array<string, mixed> $context Keys: search_query, active_category, filter_level (page filters)
     */
    public function complete(string $userMessage, array $context): ?string
    {
        ...
        if ('' === $this->apiKey) {
            $this->logger->debug('chatbot.openai.skip', ['reason' => 'empty_api_key']);
...
Ne invente pas de formations ni de prix. Les URL sont relatives au site.
...
PROMPT;
            ...
            if ($status < 200 || $status >= 300) {
                $this->logger->error('chatbot.openai.http_error', [
                    'status' => $status,
            ...
            if (!\is_array($data)) {
                $this->logger->error('chatbot.openai.invalid_json', ['preview' => mb_substr($body, 0, 300)]);
            ...
            if (!\is_string($content) || '' === trim($content)) {
                $this->logger->warning('chatbot.openai.empty_content', ['response_keys' => array_keys($data)]);

                return null;
            }

            $this->logger->info('chatbot.openai.success', [
                'model' => $this->model,
            ...
            $this->logger->error('chatbot.openai.transport', ['exception' => $e->getMessage()]);

            return null;
        } catch (\Throwable $e) {
            $this->logger->error('chatbot.openai.exception', ['exception' => $e]);
