...
final class ChatbotController extends AbstractController
{
    ...
    {
        if (!$this->isAuthorizedFormationsOrigin($request)) {
    ...
    private function isAuthorizedFormationsOrigin(Request $request): bool
    {
        $originPath = (string) $request->headers->get('X-Chat-Origin-Path', '');
        if ('' === $originPath || !str_starts_with($originPath, '/formations')) {
            return false;
        }
        ...
        if (!is_string($refererPath) || !str_starts_with($refererPath, '/formations')) {
            return false;
        }
