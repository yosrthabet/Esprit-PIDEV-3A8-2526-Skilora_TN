...
use App\Entity\User;
use App\Finance\Service\FinanceStripeClient;
use App\Finance\Service\PaymentSuccessWhatsAppMessageFactory;
use App\Finance\Service\TwilioWhatsAppNotifier;
...
final class ProjectAdvancePaymentController extends AbstractController
{
    ...
    public function createIntent(Request $request, FinanceStripeClient $stripe, UserRepository $users): JsonResponse
    {
        ...
        if ($currency === 'TND') {
            return $this->json([
                'error' => 'Le TND n\'est pas activé sur ce compte Stripe. Utilisez USD ou EUR pour les tests.',
            ], Response::HTTP_BAD_REQUEST);
        }

        if (in_array($currency, ['USD', 'EUR'], true) && $amount < 0.5) {
            return $this->json(['error' => 'Montant minimum 0,50 ' . $currency . ' en mode test Stripe.'], Response::HTTP_BAD_REQUEST);
        }
