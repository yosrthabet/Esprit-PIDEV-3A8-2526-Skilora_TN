...
final class ApplicationsTableGateway
{
     ...
     * Utile pour vérifier qu’il n’y a pas de {@code WHERE candidate_id = ?} caché : si un seul groupe,
    ...
    public function fetchEmployerCandidateBreakdown(int $ownerUserId): array
    {
        ...
        $rows = $this->connection->fetchAllAssociative(
            ...
            ORDER BY application_count DESC, a.candidate_id ASC
            SQL,
            $params,
        );
    ...
    {
        $rows = $this->connection->fetchAllAssociative(
            'SELECT * FROM applications WHERE candidate_id = ? ORDER BY applied_at DESC, id DESC',
            [$candidateId],
        );
    ...
    {
        $row = $this->connection->fetchAssociative(
            'SELECT * FROM applications WHERE job_offer_id = ? AND candidate_id = ? LIMIT 1',
            [$jobOfferId, $candidateId],
        );
