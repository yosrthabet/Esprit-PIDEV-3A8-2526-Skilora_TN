...
final class EmployerDashboardController extends AbstractController
{
    ...
    public function index(
        ...
        if ($this->employerSeesAllCandidatures) {
            $openCount = $jobOfferRepository->countOpenOffersForCandidatesFiltered();
            $all = $jobOfferRepository->findAllForEmployerViewFiltered('all');
            $totalCount = \count($all);
            $recentOffers = \array_slice($all, 0, 5);
        } elseif ($ownedCompanyIds !== [] || $ownedNamesLower !== []) {
            $openCount = $jobOfferRepository->countOpenAccessibleToEmployer($ownedCompanyIds, $ownedNamesLower);
            $all = $jobOfferRepository->findAccessibleToEmployerOrdered($ownedCompanyIds, $ownedNamesLower);
            $totalCount = \count($all);
            $recentOffers = \array_slice($all, 0, 5);
        }
        ...
        if ($uid !== null) {
            ...
            $interviewRows = $jobInterviewRepository->findEmployerInterviewListRows((int) $uid, $this->employerSeesAllCandidatures);
            foreach ($interviewRows as $row) {
                if (!$row->hasInterviewRow() || $row->scheduledAt === null) {
                    continue;
                }
                if ($row->scheduledAt > $now && $row->lifecycleStatus !== 'COMPLETED') {
                    $upcomingInterviews[] = $row;
                }
            }
            usort(
                $upcomingInterviews,
                static fn ($a, $b) => $a->scheduledAt <=> $b->scheduledAt,
            );
