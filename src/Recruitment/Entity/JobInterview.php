...
/**
 * Contrôles de saisie planification : groupe {@see self::VALIDATION_GROUP_SCHEDULE}.
 */
#[ORM\Entity(repositoryClass: JobInterviewRepository::class)]
#[ORM\Table(name: 'interviews')]
class JobInterview
{
    public const VALIDATION_GROUP_SCHEDULE = 'interview_schedule';
    ...
    #[Assert\NotNull(message: 'La candidature liée est obligatoire.', groups: [self::VALIDATION_GROUP_SCHEDULE])]
    private ?Application $application = null;

    /** Date et heure d'entretien. */
    #[ORM\Column(name: 'scheduled_date', type: Types::DATETIME_IMMUTABLE, nullable: true)]
    #[Assert\NotNull(message: 'La date et l’heure de l’entretien sont obligatoires.', groups: [self::VALIDATION_GROUP_SCHEDULE])]
    ...
    #[Assert\Choice(
        choices: [InterviewFormat::ONLINE, InterviewFormat::ONSITE],
        message: 'Type d’entretien invalide.',
        groups: [self::VALIDATION_GROUP_SCHEDULE],
    )]
    private string $format = InterviewFormat::ONLINE;

    /** Lieu de l'entretien (ex. ville, salle, lien visio si besoin). */
    #[ORM\Column(length: 150, nullable: true)]
    #[Assert\Length(max: 150, maxMessage: 'Le lieu ne peut pas dépasser {{ limit }} caractères.', groups: [self::VALIDATION_GROUP_SCHEDULE])]
    ...
    #[Assert\Length(max: 5000, maxMessage: 'La note ne peut pas dépasser {{ limit }} caractères.', groups: [self::VALIDATION_GROUP_SCHEDULE])]
    ...
    #[Assert\Callback(groups: [self::VALIDATION_GROUP_SCHEDULE])]
    public function validateLocationRequiredForOnsite(ExecutionContextInterface $context): void
    {
        if ($this->format !== InterviewFormat::ONSITE) {
            return;
        }
        if ($this->location === null || $this->location === '') {
            $context->buildViolation('Le lieu est obligatoire pour un entretien sur place.')
                ->atPath('location')
                ->addViolation();
        }
    }
